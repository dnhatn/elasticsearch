...
public class LongIntSwissHashTests extends ESTestCase {
    @ParametersFactory
    ...
    public void testValues() {
        Set<Long> values1 = randomLongValues(count);
        Set<Integer> values2 = randomIntValues(count);
        assert values1.size() == values2.size();
        long[] v1 = values1.stream().mapToLong(Long::longValue).toArray();
        int[] v2 = values2.stream().mapToInt(Integer::intValue).toArray();

        TestRecycler recycler = new TestRecycler();
        CircuitBreaker breaker = new NoopCircuitBreaker("test");
        try (LongIntSwissHash hash = new LongIntSwissHash(recycler, breaker)) {
...
//            assertThat("Only currently used pages are open", recycler.open, hasSize(expectedKeyPageCount + expectedIdPageCount));
            ...
            Integer[] iterated2 = new Integer[count];
            for (LongIntSwissHash.Itr itr = hash.iterator(); itr.next();) {
    ...
    private void assertStatus(LongIntSwissHash hash) {
    ...
    private Set<Long> randomLongValues(int count) {
    ...
    private Set<Integer> randomIntValues(int count) {
