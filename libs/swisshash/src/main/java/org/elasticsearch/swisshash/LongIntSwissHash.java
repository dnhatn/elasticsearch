...
public class LongIntSwissHash extends SwissHash implements LongIntHashTable {
     ...
     */
    @Override
    public long find(final long key1, final int key2) {
     ...
     */
    @Override
    public long add(final long key1, final int key2) {
        final int hash = hash(key1, key2);
        if (smallCore != null) {
    ...
    final class SmallCore extends Core {
        ...
        @Override
        protected LongIntSwissHash.Itr iterator() {
            return new Itr() {
                @Override
                public boolean next() {
                    return ++keyId < size;
        ...
        private int key2(int offset) {
            return (int) INT_HANDLE.get(keyPages[0], offset + Long.BYTES);
        }

        private void setKeys(int offset, long value1, long value2) {
            LONG_HANDLE.set(keyPages[0], offset, value1);
            INT_HANDLE.set(keyPages[0], offset + Long.BYTES, value2);
        }
    ...
    final class BigCore extends Core {
        ...
        @Override
        protected LongIntSwissHash.Itr iterator() {
            return new LongIntSwissHash.Itr() {
                @Override
                public boolean next() {
                    return ++keyId < size;
                }
        ...
        private long key1(final int keyOffset) {
            return (long) LONG_HANDLE.get(keyPages[keyOffset >> PAGE_SHIFT], keyOffset & PAGE_MASK);
        }

        private int key2(final int keyOffset) {
            return (int) INT_HANDLE.get(keyPages[keyOffset >> PAGE_SHIFT], (keyOffset + Long.BYTES) & PAGE_MASK);
        }

        private void setKeys(final int keyOffset, final long value1, final long value2) {
            LONG_HANDLE.set(keyPages[keyOffset >> PAGE_SHIFT], keyOffset & PAGE_MASK, value1);
            LONG_HANDLE.set(keyPages[keyOffset >> PAGE_SHIFT], (keyOffset + Long.BYTES) & PAGE_MASK, value2);
        }
    ...
    @Override
    public int getKey2(final long id) {
        final int actualId = Math.toIntExact(id);
        Objects.checkIndex(actualId, size());
