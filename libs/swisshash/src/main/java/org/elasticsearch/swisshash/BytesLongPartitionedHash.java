...
public final class BytesLongPartitionedHash implements PartitionedHashTable, Releasable {
    ...
    public PartitionedHashKeys splitPartition(CircuitBreaker breaker, PartitionSplitter partitionSplitter) {
        ...
        try {
            final Placement placement = new Placement(numIds, (int) bytesHash.find(EMPTY));
            bytesKeys = bytesHash.splitPartition(breaker, DICT_PARTITIONS, placement);
            longKeys = longlongHash.splitPartition(breaker, placement, partitionSplitter);
            final long[][] referenced = mapToLocalOrds(breaker, bytesKeys, longKeys, placement);
    ...
    /**
     * Where the dictionary split put every id, recorded from its {@link PartitionSplitter} callback, and the routing of the pairs
     * derived from it: dictionary partition in the high bits so the pair partitions sharing a slice are claimed consecutively and
     * release it early, the long in the low bits. Missing and empty bytes need no slice, so their pairs use all the bits for the
     * long; {@code emptyId} is negative when the dictionary holds no empty string.
     */
    private static final class Placement implements PartitionSplitter, LongLongSwissHash.Partitioner {
        final byte[] partitionOf;
        final int[] positionOf;
        final int emptyId;

        Placement(int numIds, int emptyId) {
            this.partitionOf = new byte[numIds];
            this.positionOf = new int[numIds];
            this.emptyId = emptyId;
        }

        @Override
        public void split(int firstId, short[] shiftedIds, int batchSize, int[] batchPartitionCounts, int[] partitionOffsets) {
            for (int p = 0; p < DICT_PARTITIONS; p++) {
                final int base = p * PARTITION_WRITE_BATCH;
                for (int i = 0; i < batchPartitionCounts[p]; i++) {
                    final int id = firstId + (shiftedIds[base + i] & 0xFFFF);
                    partitionOf[id] = (byte) p;
                    positionOf[id] = partitionOffsets[p] + i;
                }
            }
        }
        ...
        public int partition(long key1, long key2) {
            ...
            return (partitionOf[(int) key2] & 0xFF) << SUB_BITS | BitMixer.mix(key1) & SUB_MASK;
        }
    }

    /**
     * Rewrites every pair's dictionary id into the id's position within its dictionary slice and records per pair partition which
     * slice positions it references.
     */
    private static long[][] mapToLocalOrds(
        ...
        for (int p = 0; p < NUM_PARTITIONS; p++) {
            ...
            if (numKeys > 0) {
                ...
                rewriteIdsToPositions(longKeys.partitionKeys[p], numKeys, placement.positionOf, referenced[p], placement.emptyId);
            }
        }
        return referenced;
    }
