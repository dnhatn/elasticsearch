...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link AllFirstBytesRefByLongAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class AllFirstBytesRefByLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private final AllFirstBytesRefByLongAggregator.GroupingState state;

  private final List<Integer> channels;

  private final DriverContext driverContext;

  AllFirstBytesRefByLongGroupingAggregatorFunction(List<Integer> channels,
      DriverContext driverContext) {
    this.channels = channels;
    this.state = AllFirstBytesRefByLongAggregator.initGrouping(driverContext);
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, BytesRefBlock valuesBlock,
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        AllFirstBytesRefByLongAggregator.combine(state, groupId, valuesPosition, valuesBlock, timestampsBlock);
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        AllFirstBytesRefByLongAggregator.combineIntermediate(state, groupId, observed, timestampsPresent, timestamps, values, valuesPosition);
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = AllFirstBytesRefByLongAggregator.evaluateFinal(state, selectedInPage, ctx);
  }
