...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link AllLastLongByLongAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class AllLastLongByLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private final AllLastLongByLongAggregator.GroupingState state;

  private final List<Integer> channels;

  private final DriverContext driverContext;

  AllLastLongByLongGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext) {
    this.channels = channels;
    this.state = AllLastLongByLongAggregator.initGrouping(driverContext);
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, LongBlock valuesBlock,
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        AllLastLongByLongAggregator.combine(state, groupId, valuesPosition, valuesBlock, timestampsBlock);
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        AllLastLongByLongAggregator.combineIntermediate(state, groupId, observed, timestampsPresent, timestamps, values, valuesPosition);
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = AllLastLongByLongAggregator.evaluateFinal(state, selectedInPage, ctx);
  }
