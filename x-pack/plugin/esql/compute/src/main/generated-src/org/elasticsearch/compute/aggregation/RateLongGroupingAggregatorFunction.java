...
public final class RateLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    static final class Buffer implements Releasable {
        ...
        void flush(ReducedState state) {
            ...
            Slice top = pq.top();
            while (pq.size() > 0) {
                long nextTimestamp = pq.nextTimestamp();
                for (;;) {
                    position = top.next();
                    ...
                    if (top.exhausted()) {
                        top = pq.pop();
                        break;
                    } else if (top.timestamp < nextTimestamp) {
                        top = pq.updateTop();
                        break;
                    }
                }
        ...
        private SliceQueue mergeQueue() {
            var pq = new SliceQueue(this.sliceOffsets.length + 1);
            int startOffset = 0;
    ...
    static final class SliceQueue extends PriorityQueue<Slice> {
        ...
        public long nextTimestamp(){
            ...
            var heap = getHeapArray();
            Slice slice = (Slice)heap[0];
