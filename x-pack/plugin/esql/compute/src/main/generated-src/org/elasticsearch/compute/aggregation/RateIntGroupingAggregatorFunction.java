...
public final class RateIntGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    private final IntRawBuffer[] rawBuffers;
    ...
    public RateIntGroupingAggregatorFunction(
        ...
        this.rawBuffers = new IntRawBuffer[NUM_BUFFERS];
        this.reducedStates = bigArrays.newObjectArray(256);
    }

    private IntRawBuffer prepareBuffer(int groupId, int count, long firstTimestamp) {
        int idx = groupId % NUM_BUFFERS;
        IntRawBuffer buf = rawBuffers[idx];
        if (buf == null) {
            buf = new IntRawBuffer(breaker);
            rawBuffers[idx] = buf;
        }
    ...
    void flushRawBuffers() {
        for (IntRawBuffer rawBuffer : rawBuffers) {
