...
public final class RateIntGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    public static final class FunctionSupplier implements AggregatorFunctionSupplier {
        ...
        public RateIntGroupingAggregatorFunction groupingAggregator(DriverContext driverContext, List<Integer> channels) {
            return new RateIntGroupingAggregatorFunction(channels, driverContext, isRateOverTime, isDateNanos);
        }
    ...
    private final IntReducedBuffer reducedBuffer;
    private final boolean isRateOverTime;
    ...
    static final class IntReducedBuffer extends ReducedBuffer {
        private IntArray values;

        IntReducedBuffer(BigArrays bigArrays) {
            super(bigArrays);
            boolean success = false;
            try {
                this.values = bigArrays.newIntArray(512, false);
                success = true;
            } finally {
                if (success == false) {
                    close();
        ...
        void appendIntervalsFromBlocks(int groupId, long sampleCount, double reset, int valuePosition, LongBlock ts, IntBlock vs) {
            ...
            for (int i = 0; i < numIntervals; i++) {
                ...
                values.set(dst, vs.getInt(vIndex));
                timestamps.set(dst + 1, ts.getLong(tIndex + 1));
                values.set(dst + 1, vs.getInt(vIndex + 1));
            }
        }
