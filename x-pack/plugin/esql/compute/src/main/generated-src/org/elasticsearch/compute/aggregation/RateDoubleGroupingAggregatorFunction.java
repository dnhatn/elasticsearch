...
public final class RateDoubleGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    private final DoubleReducedBuffer reducedBuffer;
    private final boolean isRateOverTime;
    ...
    public RateDoubleGroupingAggregatorFunction(
        ...
        try {
            ...
            this.rawBuffer = new DoubleRawBuffer(this.bigArrays);
            this.reducedBuffer = new DoubleReducedBuffer(this.bigArrays);
            success = true;
        } finally {
            if (success == false) {
                close();
    ...
    static final class DoubleReducedBuffer extends ReducedBuffer {
        private DoubleArray values;

        DoubleReducedBuffer(BigArrays bigArrays) {
            super(bigArrays);
            boolean success = false;
            try {
                this.values = bigArrays.newDoubleArray(512, false);
                success = true;
            } finally {
                if (success == false) {
                    close();
        ...
        void appendIntervalsFromBlocks(int groupId, long sampleCount, double reset, int valuePosition, LongBlock ts, DoubleBlock vs) {
            ...
            for (int i = 0; i < numIntervals; i++) {
                ...
                values.set(dst, vs.getDouble(vIndex));
                timestamps.set(dst + 1, ts.getLong(tIndex + 1));
                values.set(dst + 1, vs.getDouble(vIndex + 1));
            }
        }
