...
class AbstractRateGroupingFunction {
    ...
    abstract static class RawBuffer implements Releasable {
        ...
        RawBuffer(CircuitBreaker breaker) {
            ...
            this.sliceOffsets = new int[INITIAL_SLICE_CAPACITY * 2];
            this.runningOffsets = new int[INITIAL_SLICE_CAPACITY];
            this.scratchBytes = (long) INITIAL_SLICE_CAPACITY * Integer.BYTES * 5;
            breaker.addEstimateBytesAndMaybeBreak(scratchBytes, "rate-slices");
        ...
        final FlushQueues prepareForFlush() {
            ...
            runningOffsets = ensureScratchCapacity(runningOffsets, numGroups);
            Arrays.fill(runningOffsets, 0, numGroups, 0);
            ...
            int requiredOffsets = sliceCount * 2;
            sliceOffsets = ensureScratchCapacity(sliceOffsets, requiredOffsets);
            for (int i = 0; i < sliceCount; i++) {
        ...
        private int[] ensureScratchCapacity(int[] array, int minLen) {
            if (array.length >= minLen) {
                return array;
            }
            int newLen = ArrayUtil.oversize(minLen, Integer.BYTES);
            long delta = (long) (newLen - array.length) * Integer.BYTES;
            breaker.addEstimateBytesAndMaybeBreak(delta, "rate-slices");
            scratchBytes += delta;
    ...
    static final class LongBuffer extends BufferedArray {
        ...
        /**
         * Fills this buffer starting at {@code dst} with values from {@code src} positions {@code from} (inclusive)
         * to {@code to} (exclusive), handling page boundaries internally.
         */
        ...
        void fill(long dst, LongVector src, int from, int to) {
    ...
    static final class DoubleBuffer extends BufferedArray {
        ...
        /**
         * Scans values from {@code from} to {@code to} and sums resets (where value > prevValue).
         * Returns the last value in the range. Accumulates resets into {@code result[0]}.
         */
