...
public enum ElementType {
    ...
    ElementType(int writeableCode, String pascalCaseName, BuilderSupplier builder, BlockReader reader) {
        ...
        this.legacyWritableName = pascalCaseName + "Block";
    ...
    private static final Map<String, Byte> legacyWritableNameToOrdinals;

    static {
        ElementType[] values = values();
        legacyWritableNameToOrdinals = Maps.newMapWithExpectedSize(values.length);
        for (int i = 0; i < values.length; i++) {
            assert values[i].writableCode == i;
            legacyWritableNameToOrdinals.put(values[i].legacyWritableName, (byte) i);
        }
    }
    ...
    static ElementType readFrom(StreamInput in) throws IOException {
        if (in.getTransportVersion().onOrAfter(TransportVersions.ESQL_SERIALIZE_BLOCK_TYPE_CODE)) {
            ...
            Byte ordinal = legacyWritableNameToOrdinals.get(writeableName);
            if (ordinal == null) {
                throw new IllegalArgumentException("Unknown element type [" + writeableName + "]");
            }
    ...
    Block readBlockOnly(BlockStreamInput in) throws IOException {
        return reader.readBlockOnly(in);
    }
