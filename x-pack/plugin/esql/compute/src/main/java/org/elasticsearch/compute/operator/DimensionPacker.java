...
public final class DimensionPacker {
    ...
    static BytesRefBlock packNulls(DriverContext driverContext, int positionCount) {
        try (var work = new BreakingBytesRefBuilder(driverContext.breaker(), "pack_dimensions", 4)) {
    ...
    public static Block[] unpackMultiColumns(DriverContext driverContext, BytesRefBlock inputBlock, ElementType[] outputTypes) {
        ...
        if (outputTypes.length == 1) {
            return new Block[] { unpackSingleColumn(driverContext, inputVector, outputTypes[0]) };
        }
