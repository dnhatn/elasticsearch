...
public final class TimeSeriesSourceOperator extends LuceneOperator {
    ...
    protected Page getCheckedOutput() throws IOException {
        ...
        try {
            ...
            processingNanos += System.nanoTime() - startTimeInNanos;
        }
        return page;
    }
    ...
    protected void describe(StringBuilder sb) {
        ...
        sb.append(", fields = ").append(fieldsToExtracts.stream().map(ValuesSourceReaderOperator.FieldInfo::name).toList());
    }

    @Override
    public Status status() {
        return new Status(this, tsidLoaded, rowsEmitted * fieldsToExtracts.size(), 0L);
    }
    ...
    public static class Status extends LuceneOperator.Status {
        ...
        private final long valuedLoadedNanos;

        Status(TimeSeriesSourceOperator operator, long tsidLoaded, long valuesLoaded, long valuedLoadedNanos) {
            super(operator);
            this.tsidLoaded = tsidLoaded;
            this.valuesLoaded = valuesLoaded;
            this.valuedLoadedNanos = valuedLoadedNanos;
        }
        ...
        Status(StreamInput in) throws IOException {
            ...
            this.valuedLoadedNanos = in.readVLong();
        }
        ...
        public void writeTo(StreamOutput out) throws IOException {
            ...
            out.writeVLong(valuedLoadedNanos);
        }

        @Override
        protected void doToXContent(XContentBuilder builder, Params params) throws IOException {
            super.doToXContent(builder, params);
            builder.field("tsid_loaded", tsidLoaded);
            builder.field("values_loaded", valuesLoaded);
            builder.field("values_loaded_nanos", valuedLoadedNanos);
            if (builder.humanReadable()) {
                builder.field("values_loaded_time", TimeValue.timeValueNanos(valuedLoadedNanos));
            }
        }
