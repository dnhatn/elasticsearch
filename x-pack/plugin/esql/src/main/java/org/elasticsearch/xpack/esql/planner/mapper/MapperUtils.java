...
public class MapperUtils {
    ...
    static PhysicalPlan mapUnary(UnaryPlan p, PhysicalPlan child) {
        ...
        if (p instanceof PackDimensions pack) {
            return new PackDimensionsExec(pack.source(), child, pack.dimensions(), pack.packedAttribute());
        }

        if (p instanceof UnpackDimensionValues unpack) {
            return new UnpackDimensionValuesExec(unpack.source(), child, unpack.packed(), unpack.unpackedDimensions());
        }
