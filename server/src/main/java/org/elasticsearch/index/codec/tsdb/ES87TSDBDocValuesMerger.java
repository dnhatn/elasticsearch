...
class ES87TSDBDocValuesMerger extends ES87TSDBDocValuesConsumer {

    ES87TSDBDocValuesMerger(SegmentWriteState state, String dataCodec, String dataExtension, String metaCodec, String metaExtension)
        throws IOException {
    ...
    public void mergeNumericField(final FieldInfo mergeFieldInfo, final MergeState mergeState) throws IOException {
        addNumericField(mergeFieldInfo, new EmptyDocValuesProducer() {
            ...
            public NumericDocValues getNumeric(FieldInfo fieldInfo) throws IOException {
                ...
                return mergeNumericValues(mergeState, fieldInfo, subs, mergeState.needsIndexSort);
            }
        });
    }
    ...
    private static NumericDocValues mergeNumericValues(
        ...
        return new NumericDocValuesWithCount() {
            private int docID = -1;
            private NumericDocValuesSub current;
            private int numDocsWithValue = -1;

            private int getNumDocsWithValues() throws IOException {
                int numDocs = 0;
                for (int i = 0; i < mergeState.docValuesProducers.length; i++) {
                    DocValuesProducer docValuesProducer = mergeState.docValuesProducers[i];
                    if (docValuesProducer == null) {
                    ...
                    final NumericDocValues numeric;
                    if (field.getDocValuesType() == DocValuesType.NUMERIC) {
                        numeric = docValuesProducer.getNumeric(field);
                    } else {
                        // TODO: fix me
                        numeric = DocValues.unwrapSingleton(docValuesProducer.getSortedNumeric(field));
                        assert numeric != null;
                    }
                    if (numeric == null) {
                        continue;
                    ...
                    if (liveDocs == null && numeric instanceof WithDocValuesCount wc) {
                        numDocs += wc.numDocsWithValues();
                    } else {
                        int docID;
                        while ((docID = numeric.nextDoc()) != NO_MORE_DOCS) {
            ...
            public int numDocsWithValues() throws IOException {
                if (numDocsWithValue == -1) {
                    numDocsWithValue = getNumDocsWithValues();
                }
        ...
        addSortedNumericField(mergeFieldInfo, new EmptyDocValuesProducer() {
            ...
            public SortedNumericDocValues getSortedNumeric(FieldInfo fieldInfo) throws IOException {
                ...
                if (allSingletons) {
                    ...
                    return DocValues.singleton(mergeNumericValues(mergeState, fieldInfo, singleValuedSubs, mergeState.needsIndexSort));
                }
    ...
    public void mergeSortedField(FieldInfo fieldInfo, final MergeState mergeState) throws IOException {
        ...
        addSortedField(fieldInfo, new EmptyDocValuesProducer() {
            ...
            public SortedDocValues getSorted(FieldInfo fieldInfoIn) throws IOException {
                ...
                return mergeSortedValues(mergeState, fieldInfo, subs, mergeState.needsIndexSort, map);
            }
        });
    }
    ...
    private static SortedDocValues mergeSortedValues(
        ...
        return new SortedDocValuesWithCount() {
            private int docID = -1;
            ...
            private int getNumDocsWithValues() throws IOException {
                ...
                for (int i = 0; i < mergeState.docValuesProducers.length; i++) {
                    ...
                    SortedDocValues sorted = docValuesProducer.getSorted(field);
                    if (sorted == null) {
                        continue;
                    }
                    Bits liveDocs = mergeState.liveDocs[i];
                    if (liveDocs == null && sorted instanceof WithDocValuesCount wc) {
                        numDocs += wc.numDocsWithValues();
                    } else {
                        int docID;
        ...
        addSortedSetField(mergeFieldInfo, new EmptyDocValuesProducer() {
            ...
            public SortedSetDocValues getSortedSet(FieldInfo fieldInfo) throws IOException {
                ...
                if (allSingletons) {
                    ...
                    return DocValues.singleton(mergeSortedValues(mergeState, fieldInfo, singleValuedSubs, mergeState.needsIndexSort, map));
                }
