...
public final class Lucene90CompressingStoredFieldsWriter extends StoredFieldsWriter {
    ...
    Lucene90CompressingStoredFieldsWriter(
        ...
        try {
            ...
            indexWriter = new FieldsIndexWriter(
                directory,
                segment,
                segmentSuffix,
                INDEX_EXTENSION,
    ...
    private void writeHeader(int docBase, int numBufferedDocs, int[] numStoredFields, int[] lengths, boolean sliced, boolean dirtyChunk)
        throws IOException {
    ...
    private void copyChunks(final MergeState mergeState, final CompressingStoredFieldsMergeSub sub, final int fromDocID, final int toDocID)
        throws IOException {
    ...
    boolean tooDirty(Lucene90CompressingStoredFieldsReader candidate) {
        ...
        return candidate.getNumDirtyDocs() > maxDocsPerChunk && candidate.getNumDirtyChunks() * 100 > candidate.getNumChunks();
    }
    ...
    public long ramBytesUsed() {
        return bufferedDocs.ramBytesUsed() + numStoredFields.length * (long) Integer.BYTES + endOffsets.length * (long) Integer.BYTES;
    }
}
