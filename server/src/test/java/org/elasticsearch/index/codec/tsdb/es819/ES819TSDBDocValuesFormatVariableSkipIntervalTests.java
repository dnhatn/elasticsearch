...
public class ES819TSDBDocValuesFormatVariableSkipIntervalTests extends ES87TSDBDocValuesFormatVariableSkipIntervalTests {
    ...
    protected Codec getCodec() {
        ...
        return TestUtil.alwaysDocValuesFormat(new ES819TSDBDocValuesFormat(f -> null, random().nextInt(4, 16), random().nextBoolean()));
    }

    public void testSkipIndexIntervalSize() {
        IllegalArgumentException ex = expectThrows(
            IllegalArgumentException.class,
            () -> new ES819TSDBDocValuesFormat(f -> null, random().nextInt(Integer.MIN_VALUE, 2), random().nextBoolean())
        );
